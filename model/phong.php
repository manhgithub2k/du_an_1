...
function getAvailableRooms($checkinTime, $checkoutTime, $idLoaiPhong) {
    ...
    foreach($activeRooms as $row) {
        ...
        if(
            ...
            ($checkinTime > $row['ngay_checkin'] && $checkinTime < $row['ngay_checkout']) ||
            ($checkoutTime > $row['ngay_checkin'] && $checkoutTime < $row['ngay_checkout']) ||
            ($checkinTime < $row['ngay_checkin'] && $checkoutTime > $row['ngay_checkout'])
